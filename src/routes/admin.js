...
router.get('/stats', adminAuth, async (req, res) => {
  try {
    const [activeRes, frozenRes, newsletterRes] = await Promise.all([
      ...
      db.query(`SELECT id, subject, sent_to, sent_at FROM newsletters ORDER BY sent_at DESC LIMIT 5`),
    ]);
...
router.post('/preview', adminAuth, async (req, res) => {
  try {
    const topics = (process.env.NEWSLETTER_TOPICS || 'AI news')
      .split(',').map(t => t.trim());
...
// PATCH /admin/subscriber/:email/freeze
...
// PATCH /admin/subscriber/:email/activate
router.patch('/subscriber/:email/activate', adminAuth, async (req, res) => {
  try {
    await db.query(
      `UPDATE subscribers SET status = 'active', frozen_at = NULL WHERE email = $1`,
      [req.params.email]
    );
    res.json({ success: true });
  } catch (err) {
