...
// ── ONE-TIME: remove repeat soft bouncers from ddn-free tag ──────────────────
router.post('/clean-soft-bounces', adminAuth, async (req, res) => {
  res.json({ message: 'Soft bounce cleanup started — check Render logs for results' });
  ...
  (async () => {
    try {
      ...
      console.log(`🧹 Soft bounce cleanup: ${rows.length} emails to process`);
      let cleaned = 0, notFound = 0, failed = 0;

      for (const { email, bounce_count } of rows) {
        try {
          const contactId = await lookupContact(email);
          if (!contactId) { notFound++; continue; }
          ...
          await axios.post(`${GHL_BASE}/contacts/${contactId}/tags`, { tags: ['soft-bounced-ddn-free'] }, {
            headers: { Authorization: `Bearer ${apiKey}`, Version: '2021-04-15', 'Content-Type': 'application/json' },
          });

          console.log(`✅ Soft bounce removed (${bounce_count}x): ${email}`);
          cleaned++;
      ...
      console.log(`✅ Soft bounce cleanup done — removed: ${cleaned}, not in GHL: ${notFound}, failed: ${failed}`);
    } catch (err) {
      console.error('❌ Soft bounce cleanup error:', err.message);
    }
  })();
});
